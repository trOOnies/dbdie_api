...
@router.get("/filter-with-dbdvr", response_model=list[PerkOut])
def filter_perks_by_dbdvr(
    dbdv_min_id: int,
    dbdv_max_id: int | None = None,
    db: "Session" = Depends(get_db),
):
    return filter_with_dbdvr(db, Perk, dbdv_min_id, dbdv_max_id)


@router.get("/filter-with-dbdvr/count", response_model=int)
def count_perks_by_dbdvr(
    dbdv_min_id: int,
    dbdv_max_id: int | None = None,
    db: "Session" = Depends(get_db),
):
    return count_with_dbdvr(db, Perk, dbdv_min_id, dbdv_max_id)
